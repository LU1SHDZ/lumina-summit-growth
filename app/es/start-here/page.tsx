...
export default function SpanishStartHerePage() {
  return <main id="main-content" lang="es">
    ...
    <OfferQuickNav locale="es" />

    <section id="fit" className="scroll-mt-16 px-6 py-24 lg:px-8 lg:py-32"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2 lg:gap-16"><div><p className="eyebrow">Para quién es</p><h2 className="mt-5 text-pretty font-display text-5xl leading-[.88] sm:text-6xl">Una base para negocios preparados para ver todo el sistema.</h2><p className="mt-7 max-w-xl text-base leading-7 text-charcoal/70">El Sprint está diseñado para operadores que quieren una visión honesta de dónde se pierde la oportunidad antes de comprometerse con más tácticas, herramientas o publicidad.</p></div><ul className="divide-y divide-charcoal/15 border-t border-charcoal/15">{localGrowthFoundationEs.fitSignals.map((signal, index)=><li key={signal} className="flex gap-5 py-5 text-sm leading-6 text-charcoal/75"><span className="font-display text-2xl text-terracotta">0{index+1}</span>{signal}</li>)}</ul></div></section>

    <section id="evaluation" className="scroll-mt-16 bg-[#eee7d9] px-6 py-24 lg:px-8 lg:py-32"><div className="mx-auto max-w-7xl"><p className="eyebrow">Lo que evaluamos</p><div className="mt-8 grid border-l border-t border-charcoal/15 sm:grid-cols-2 lg:grid-cols-3">{localGrowthFoundationEs.evaluationAreas.map((area,index)=><article key={area} className="min-h-36 border-b border-r border-charcoal/15 p-5 sm:min-h-40 sm:p-6"><span className="font-display text-2xl text-terracotta">{String(index+1).padStart(2,"0")}</span><h2 className="mt-7 text-pretty font-display text-3xl leading-none">{area}</h2></article>)}</div></div></section>

    <section id="process" className="scroll-mt-16 bg-agave px-6 py-24 text-cream lg:px-8 lg:py-32"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.65fr_1.35fr] lg:gap-16"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-gold">El proceso</p><h2 className="mt-5 text-pretty font-display text-5xl leading-[.88] sm:text-6xl">Escuchar primero.<br />Luego iluminar el camino.</h2></div><div>{localGrowthFoundationEs.process.map((phase)=><article key={phase.number} className="grid gap-4 border-t border-cream/20 py-7 sm:grid-cols-[.12fr_.3fr_.58fr]"><span className="font-display text-2xl text-gold">{phase.number}</span><h3 className="font-display text-3xl">{phase.title}</h3><p className="text-sm leading-6 text-cream/70">{phase.description}</p></article>)}</div></div></section>

    <section id="deliverables" className="scroll-mt-16 px-6 py-24 lg:px-8 lg:py-32"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2 lg:gap-16"><div><p className="eyebrow">Entregables esperados</p><h2 className="mt-5 text-pretty font-display text-5xl leading-[.88] sm:text-6xl">Evidencia, prioridades y una hoja de ruta que tu equipo puede usar.</h2></div><ul className="space-y-3">{localGrowthFoundationEs.deliverables.map((item)=><li key={item} className="flex gap-4 border-l-2 border-gold bg-[#eee7d9] p-5 text-sm leading-6 text-charcoal/75"><span aria-hidden="true">✓</span>{item}</li>)}</ul></div></section>
