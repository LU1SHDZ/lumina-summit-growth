...
export default function SpanishFreeAuditPage() {
  return <main id="main-content" lang="es" className="min-h-screen bg-[#eee7d9]">
    <div className="px-6 py-7 lg:px-8"><div className="mx-auto max-w-7xl">
      ...
      <div className="grid min-w-0 gap-14 py-16 lg:grid-cols-[.75fr_1.25fr] lg:py-24"><section className="min-w-0 lg:sticky lg:top-10 lg:self-start"><p className="eyebrow">Auditoría de crecimiento / Solicitud de calificación</p><h1 className="mt-6 font-display text-5xl leading-[.84] tracking-[-.035em] sm:text-7xl">Contexto antes<br/>de recomendaciones.</h1><p className="mt-8 max-w-md text-base leading-7 text-charcoal/70">Esta solicitud nos ayuda a entender tu negocio y decidir si el Sprint de Fundamentos de Crecimiento Local es un siguiente paso responsable.</p><div className="mt-10 space-y-5 border-t border-charcoal/15 pt-7">{["Revisada por una persona, no calificada por un algoritmo sin validar", "Utilizada para preparar una primera conversación más útil", "Sin garantía, obligación ni secuencia automática de ventas"].map((item)=><p key={item} className="flex items-start gap-3 text-sm leading-6 text-charcoal/75"><Check size={16} className="mt-1 shrink-0 text-terracotta"/>{item}</p>)}</div><p className="mt-8 text-xs leading-5 text-charcoal/50">El formulario pregunta sobre inversión, tamaño del equipo y toma de decisiones únicamente para entender la compatibilidad. Los criterios finales de calificación aún no se han establecido.</p></section><FreeAuditForm locale="es"/></div>
