...
export const metadata: Metadata = {
  title: "Start Here | Local Growth Foundation",
  description: "See how the Local Growth Foundation helps established service businesses repair visibility, conversion, tracking, and lead-generation foundations.",
...
export default function StartHerePage() {
  return <main id="main-content">
    ...
    <section className="relative overflow-hidden bg-charcoal px-6 pb-24 pt-40 text-cream lg:px-8 lg:pb-32"><div className="grain absolute inset-0 opacity-20" /><SiteHeader /><div className="relative mx-auto max-w-7xl"><p className="text-xs font-semibold uppercase tracking-[.18em] text-gold">A focused foundation project</p><h1 className="mt-6 max-w-5xl font-display text-6xl leading-[.82] tracking-[-.04em] sm:text-8xl">Local Growth<br /><em className="font-normal text-gold">Foundation.</em></h1><p className="mt-9 max-w-2xl text-lg leading-8 text-cream/70">{localGrowthFoundation.summary}</p><div className="mt-8 border-l-2 border-gold pl-5"><p className="font-display text-3xl text-gold">{localGrowthFoundation.pricing.display}</p><p className="mt-2 max-w-xl text-xs leading-5 text-cream/50">{localGrowthFoundation.pricing.note}</p></div><div className="mt-10 flex flex-wrap gap-4"><Link href="/free-audit" className="bg-gold px-6 py-4 text-xs font-bold uppercase tracking-[.14em] text-charcoal hover:bg-cream">Request a Free Growth Snapshot →</Link><a href="#fit" className="border border-cream/25 px-6 py-4 text-xs font-bold uppercase tracking-[.14em] hover:border-gold">Check the fit</a></div></div></section>

    <OfferQuickNav />

    <section id="fit" className="scroll-mt-16 px-6 py-24 lg:px-8 lg:py-32"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-2 lg:gap-16"><div><p className="eyebrow">Who it is for</p><h2 className="mt-5 text-pretty font-display text-5xl leading-[.88] sm:text-6xl">A foundation for businesses ready to see the whole system.</h2><p className="mt-7 max-w-xl text-base leading-7 text-charcoal/70">The Foundation is designed for operators who want to repair the constraints limiting visibility and conversion before committing to more tactics, tools, or advertising.</p></div><ul className="divide-y divide-charcoal/15 border-t border-charcoal/15">{localGrowthFoundation.fitSignals.map((signal, index)=><li key={signal} className="flex gap-5 py-5 text-sm leading-6 text-charcoal/75"><span className="font-display text-2xl text-terracotta">0{index+1}</span>{signal}</li>)}</ul></div></section>
