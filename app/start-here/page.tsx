...
export default function StartHerePage() {
  return <main id="main-content">
    ...
    <section id="fit" className="px-6 py-24 lg:px-8 lg:py-32"><div className="mx-auto grid max-w-7xl gap-16 lg:grid-cols-2"><div><p className="eyebrow">Who it is for</p><h2 className="mt-5 font-display text-6xl leading-[.88]">A foundation for businesses ready to see the whole system.</h2><p className="mt-7 max-w-xl text-base leading-7 text-charcoal/70">The Sprint is designed for operators who want a candid view of where opportunity is being lost before committing to more tactics, tools, or advertising.</p></div><ul className="divide-y divide-charcoal/15 border-t border-charcoal/15">{localGrowthFoundation.fitSignals.map((signal, index)=><li key={signal} className="flex gap-5 py-5 text-sm leading-6 text-charcoal/75"><span className="font-display text-2xl text-terracotta">0{index+1}</span>{signal}</li>)}</ul></div></section>

    <section className="bg-[#eee7d9] px-6 py-24 lg:px-8 lg:py-32"><div className="mx-auto max-w-7xl"><p className="eyebrow">What we evaluate</p><div className="mt-8 grid border-l border-t border-charcoal/15 sm:grid-cols-2 lg:grid-cols-3">{localGrowthFoundation.evaluationAreas.map((area,index)=><article key={area} className="min-h-44 border-b border-r border-charcoal/15 p-6"><span className="font-display text-2xl text-terracotta">0{index+1}</span><h2 className="mt-10 font-display text-3xl leading-none">{area}</h2></article>)}</div></div></section>

    <section className="bg-agave px-6 py-24 text-cream lg:px-8 lg:py-32"><div className="mx-auto grid max-w-7xl gap-16 lg:grid-cols-[.65fr_1.35fr]"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-gold">The process</p><h2 className="mt-5 font-display text-6xl leading-[.88]">Listen first.<br />Then illuminate the path.</h2></div><div>{localGrowthFoundation.process.map((phase)=><article key={phase.number} className="grid gap-4 border-t border-cream/20 py-7 sm:grid-cols-[.12fr_.3fr_.58fr]"><span className="font-display text-2xl text-gold">{phase.number}</span><h3 className="font-display text-3xl">{phase.title}</h3><p className="text-sm leading-6 text-cream/70">{phase.description}</p></article>)}</div></div></section>

    <section className="px-6 py-24 lg:px-8 lg:py-32"><div className="mx-auto grid max-w-7xl gap-16 lg:grid-cols-2"><div><p className="eyebrow">Expected deliverables</p><h2 className="mt-5 font-display text-6xl leading-[.88]">Evidence, priorities, and a roadmap your team can use.</h2></div><ul className="space-y-4">{localGrowthFoundation.deliverables.map((item)=><li key={item} className="flex gap-4 border-l-2 border-gold bg-[#eee7d9] p-5 text-sm leading-6 text-charcoal/75"><span aria-hidden="true">✓</span>{item}</li>)}</ul></div></section>
    ...
    <section className="bg-[#eee7d9] px-6 py-24 lg:px-8"><div className="mx-auto max-w-4xl"><p className="eyebrow">Frequently asked questions</p><h2 className="mt-5 font-display text-6xl">Before you apply.</h2><div className="mt-10 divide-y divide-charcoal/15 border-y border-charcoal/15">{foundationFaq.map((item)=><details key={item.question} className="group py-6"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 font-display text-2xl"><span>{item.question}</span><span aria-hidden="true" className="text-terracotta group-open:rotate-45">+</span></summary><p className="mt-4 max-w-2xl text-sm leading-7 text-charcoal/70">{item.answer}</p></details>)}</div></div></section>
