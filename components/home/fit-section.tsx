...
export function FitSection({ locale = "en" }: { locale?: Locale }) {
  ...
  return <section className="border-y border-charcoal/10 bg-cream px-6 py-16 lg:px-8"><div className="mx-auto grid max-w-7xl gap-9 lg:grid-cols-[.55fr_1.45fr]"><div><p className="eyebrow">{copy.eyebrow}</p><h2 className="mt-4 max-w-md text-pretty font-display text-4xl leading-none">{copy.title}</h2></div><div className="grid gap-px bg-charcoal/15 md:grid-cols-3">{copy.signals.map((signal, index) => <p key={signal} className="bg-cream p-5 text-sm leading-6 text-charcoal/70 sm:p-6"><span className="mb-3 block font-display text-xl text-terracotta">0{index + 1}</span>{signal}</p>)}</div></div></section>;
}
