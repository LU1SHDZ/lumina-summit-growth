...
export function Homepage({ locale = "en" }: { locale?: Locale }) {
  ...
  return (
    <main id="main-content" lang={locale} className="overflow-hidden">
      ...
      <section id="audit" className="scroll-mt-6 bg-terracotta px-6 py-20 text-cream lg:px-8 lg:py-24"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.2fr_.8fr] lg:items-end lg:gap-12"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-gold">{spanish ? "Un primer paso práctico" : "A practical first step"}</p><h2 className="mt-5 text-pretty font-display text-5xl leading-[.84] sm:text-7xl">{spanish ? <>Encuentra la limitación<br />que frena tu crecimiento.</> : <>Find the constraint<br />slowing your growth.</>}</h2></div><div><p className="max-w-md text-base leading-7 text-cream/80">{spanish ? "Solicita una primera revisión de tu sitio, visibilidad local y recorrido de conversión. Recibirás algunas observaciones útiles, sin confundirlas con una auditoría completa o una hoja de ruta gratuita." : "Request a focused first look at your website, local visibility, and conversion path. You’ll receive a few useful observations—without confusing them for a complete audit or free implementation roadmap."}</p><a href={localizedPath("/free-audit", locale)} className="mt-8 inline-flex min-h-12 w-full items-center justify-center gap-3 bg-cream px-6 py-4 text-center text-xs font-bold uppercase tracking-[.14em] text-charcoal transition hover:bg-gold sm:w-auto">{spanish ? "Solicita tu Diagnóstico Inicial gratuito →" : "Request your free Growth Snapshot →"}</a></div></div></section>
